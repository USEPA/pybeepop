...
def test_init_default_lib(monkeypatch):
    ...
    class DummyBeePopModel:
        ...
        def set_parameters(self, p): return p
        def get_parameters(self): return {"icworkeradults": 1000}
        def load_weather(self, f): pass
        def load_input_file(self, f): pass
        def load_contam_file(self, f): pass
        def run_beepop(self): import pandas as pd; return pd.DataFrame({"Colony Size": [1], "Date": ["Initial"]})
        def get_errors(self): return "No errors"
        def get_info(self): return "Info"
        def get_version(self): return "1.0"
        def close_library(self): pass
...
def test_set_parameters_and_get(monkeypatch):
    ...
    class DummyBeePopModel:
        def __init__(self, *a, **k): pass
        def set_parameters(self, p): return p
        def get_parameters(self): return {"icworkeradults": 123}
    monkeypatch.setattr(os.path, "isfile", lambda x: True)
    monkeypatch.setattr(pbp, "BeePopModel", DummyBeePopModel)
    beepop = PyBeePop()
...
def test_run_model_no_weather(monkeypatch):
    ...
    class DummyBeePopModel:
        ...
        def set_parameters(self, p): return p
        def get_parameters(self): return {}
        def run_beepop(self): return None
    monkeypatch.setattr(os.path, "isfile", lambda x: True)
    monkeypatch.setattr(pbp, "BeePopModel", DummyBeePopModel)
    beepop = PyBeePop()
...
def test_get_output_json(monkeypatch):
    ...
    class DummyBeePopModel:
        ...
        def set_parameters(self, p): return p
        def get_parameters(self): return {}
        def run_beepop(self): return pd.DataFrame({"Colony Size": [1], "Date": ["Initial"]})
    monkeypatch.setattr(os.path, "isfile", lambda x: True)
    monkeypatch.setattr(pbp, "BeePopModel", DummyBeePopModel)
    beepop = PyBeePop()
...
def test_plot_output_invalid_column(monkeypatch):
    ...
    class DummyBeePopModel:
        ...
        def set_parameters(self, p): return p
        def get_parameters(self): return {}
    def dummy_plot_timeseries(output, columns): return None
    monkeypatch.setattr(os.path, "isfile", lambda x: True)
    monkeypatch.setattr(pbp, "BeePopModel", DummyBeePopModel)
...
def test_get_error_and_info_log(monkeypatch):
    ...
    class DummyBeePopModel:
        def __init__(self, *a, **k): pass
        def get_errors(self): return "error log"
        def get_info(self): return "info log"
    monkeypatch.setattr(os.path, "isfile", lambda x: True)
    monkeypatch.setattr(pbp, "BeePopModel", DummyBeePopModel)
    beepop = PyBeePop()
...
def test_version_and_exit(monkeypatch):
    ...
    class DummyBeePopModel:
        def __init__(self, *a, **k): pass
        def get_version(self): return "2.1"
        def close_library(self): self.closed = True
    monkeypatch.setattr(os.path, "isfile", lambda x: True)
    monkeypatch.setattr(pbp, "BeePopModel", DummyBeePopModel)
    beepop = PyBeePop()
